...
const ReponsiveLogoNavBar = ( props ) => {
  ...
  <nav>
    ...
    <ul className="nav-list">
      {
        props.map( link => <li className="nav-item"><Link to={link.link}> { link.title } </Link></li> )
      }
    </ul>
    <div className="nav-mobile-btn" id="mobile-btn" onclick={ () => { displayMenu() } } >
      ...
      {
        props.map( link => <li className="mobile-nav-item"><Link to={link.link}> { link.title } </Link></li> )
      }
      </ul>
      </div>
  </nav>
...
export default ReponsiveLogoNavBar
